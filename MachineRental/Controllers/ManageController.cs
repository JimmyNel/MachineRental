...
namespace MachineRental.Controllers
{
    ...
    public class ManageController : Controller
    {
        ...
        [AllowAnonymous]
        public ActionResult SetPassword(string userEmail)
        {
        ...
        public async Task<ActionResult> SetPassword(SetPasswordViewModel model)
        {
            if (ModelState.IsValid)
            {
                if (model.UserEmail != null)
                {
                    ...
                    if (user != null)
                    {
                        var result = await UserManager.ChangePasswordAsync(user.Id, Constant.DefaultPassword, model.NewPassword);
                        if (result.Succeeded)
                        {
                ...
                ViewBag.StatusMessage = Constant.ErrorSetPassword;
            }
        ...
        [Authorize(Roles = Constant.RentalManager + ", " + Constant.Admin)]
        ...
        public ActionResult EditUser(string id)
        {
            ...
            var userInRole = (from user in _context.Users
                              where user.Id == id
                              select new
                              {
                                  UserId = user.Id,
            ...
            return View("EditUser", userInRole);
        }

        [Authorize(Roles = Constant.RentalManager + ", " + Constant.Admin)]
        // POST: Manage/SaveUser/{id}
        [HttpPost]
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]  // Prevent Cross Site Request Forgery (CSRF) Attacks by generating and comparing Token automatically
        public async Task<ActionResult> GenerateEmailConfirmation(string id)
        {
            if (id == null)
                return new HttpStatusCodeResult(HttpStatusCode.BadRequest);
