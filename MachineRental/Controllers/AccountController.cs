...
namespace MachineRental.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var userConnected = await UserManager.FindByEmailAsync(model.Email);
            ...
            switch (result)
            {
                case SignInStatus.Success:
                    ...
                    {
                        ModelState.AddModelError("", "Utilisateur non actif");
                        AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
                        return View(model);
                    }
                    // check if user has confirmed the email
                    if (!(await UserManager.IsEmailConfirmedAsync(userConnected.Id)))
                    {
                        ModelState.AddModelError("", "Utilisateur non confirmé");
                        AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
                        return View(model);
                    }
                    // check if user has set a password
                    if (userConnected.HasPasswordSet == false)
                    {
                        AuthenticationManager.SignOut(DefaultAuthenticationTypes.ApplicationCookie);
                        return RedirectToAction("SetPassword", "Manage", new { userEmail = model.Email});
                    }
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, IsEnabled = true, HasPasswordSet = false };
                var result = await UserManager.CreateAsync(user, Constant.DefaultPassword);
                if (result.Succeeded)
                {
                    // sign in when register success
                    // await SignInManager.SignInAsync(user, isPersistent:false, rememberBrowser:false);

                    // assign user to the selected role in DB
                    await UserManager.AddToRoleAsync(user.Id, _context.Roles.Single(r => r.Id == model.RoleId).Name);
                ...
                {
                    ModelState.AddModelError("", "Utilisateur déja enregistré");
                    //AddErrors(result);
                    var vm = new RegisterViewModel
                    {
                        Email = model.Email,
        ...
        public async Task<ActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (result.Succeeded)
            {
                // sign in user after email confirmation in order to be authorized to Set Password
                //var user = await UserManager.FindByIdAsync(userId);
                //if (user != null)
                //{
                //    await SignInManager.SignInAsync(user, isPersistent: false, rememberBrowser: false);
                //}
                return RedirectToAction("SetPassword", "Manage", new { UserEmail = user.Email });
            }
            return View("Error");
        }
